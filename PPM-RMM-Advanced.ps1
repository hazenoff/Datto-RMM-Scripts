...
# =============================================================================
# PPM RMM -- Paladin Performance Maximizer (Advanced Tier)
# Datto RMM Component: PPM RMM Advanced [WIN]
# Version: 9.0.0
# Context: NT AUTHORITY\SYSTEM (LocalSystem)
# Tier: Advanced -- all Safe + CPU, memory, disk, NIC, GPU tuning.
...
# Dangerous tweaks (MSI Mode, NativeNVMe) excluded -- use Dangerous Tweaks tier.
...
Write-Host "PPM RMM Advanced v9.0.0 | Site: $siteName | $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
Write-Host 'Reboot recommended after apply.'
...
# NetAdapterRss -- try import only, no DISM/feature install
...
# Optimize-Volume -- try import only, no DISM/feature install
...
# LSA P/Invoke for SeLockMemoryPrivilege fallback (secedit unavailable)
$lsaSource = @'
...
function Backup-RegValue {
    ...
    try { Ensure-BackupRoot; $sk=($Path+'_'+$Name)-replace'[\\:/]','_'; $e=Get-ItemProperty -Path $Path -Name $Name -EA SilentlyContinue; $bv=if($null -ne $e){$e.$Name}else{'__NOTEXIST__'}; New-ItemProperty -Path $BackupRoot -Name $sk -Value $bv -Force -EA SilentlyContinue|Out-Null } catch {}
}
...
# 01 -- Power Plan
Write-Host '-- [01/17] Power Plan (High Performance)'
try {
    ...
    elseif(-not $Undo){
        ...
        if($null -ne $b){& powercfg /setactive $b.PowerPlan_Active 2>&1|Out-Null;Write-Host '  OK: Plan restored';$ok++}else{Write-Host '  WARN: No backup';$warn++}
    }
}catch{Write-Host "  ERROR: PowerPlan -- $($_.Exception.Message)";$err++}

# 02 -- Win32PrioritySeparation
Write-Host '-- [02/17] Win32PrioritySeparation'
try {
    $p='HKLM:\SYSTEM\CurrentControlSet\Control\PriorityControl'
    if(-not $Undo){Backup-RegValue -Path $p -Name 'Win32PrioritySeparation';Set-RegDWord -Path $p -Name 'Win32PrioritySeparation' -Value 38;Write-Host '  OK: =38';$ok++}
...
# 03 -- Visual Effects
Write-Host '-- [03/17] Visual Effects (performance mode)'
try {
    $p='HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects'
    if(-not $Undo){Backup-RegValue -Path $p -Name 'VisualFXSetting';Set-RegDWord -Path $p -Name 'VisualFXSetting' -Value 2;Write-Host '  OK: =2';$ok++}
...
# 04 -- MMCSS
Write-Host '-- [04/17] MMCSS NetworkThrottlingIndex + SystemResponsiveness + Games task'
try {
    ...
    }else{Restore-RegValue -Path $mm -Name 'NetworkThrottlingIndex' -Type 'DWord';Restore-RegValue -Path $mm -Name 'SystemResponsiveness' -Type 'DWord';Write-Host '  OK: MMCSS restored';$ok++}
}catch{Write-Host "  ERROR: MMCSS -- $($_.Exception.Message)";$err++}

# 05 -- NTFS
Write-Host '-- [05/17] NTFS overhead (8.3, LastAccess, DisablePagingExecutive)'
try {
    ...
    if(-not $Undo){
        ...
        else{Set-RegDWord -Path $np -Name 'NtfsDisable8dot3NameCreation' -Value 1;Set-RegDWord -Path $np -Name 'NtfsDisableLastAccessUpdate' -Value 1;Write-Host '  INFO: Registry fallback'}
...
# 06 -- Nagle
Write-Host '-- [06/17] Nagle disable + TcpAckFrequency'
try {
...
# 07 -- Core Parking
Write-Host '-- [07/17] Core Parking (unpark all cores)'
try {
    if(-not $hasPowercfg){Write-Host '  SKIP: powercfg missing';$warn++}
    elseif(-not $Undo){
...
# 08 -- Pagefile (fixed size, RAM-bounded)
Write-Host '-- [08/17] Pagefile (fixed size)'
try {
    $pfMB=[math]::Min($ramMB,8192)
    $pfP='HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management'
    ...
    else{Restore-RegValue -Path $pfP -Name 'PagingFiles' -Type 'MultiString';Write-Host '  OK: Pagefile restored';$ok++}
}catch{Write-Host "  ERROR: Pagefile -- $($_.Exception.Message)";$err++}

# 09 -- Hibernate (workstation only)
Write-Host '-- [09/17] Hibernate disable (workstation only)'
try {
    if($isServer){Write-Host '  SKIP: SERVER OS';$warn++}
    ...
    elseif(-not $Undo){& powercfg /h off 2>&1|Out-Null;Write-Host '  OK: Hibernate disabled';$ok++}
    else{& powercfg /h on 2>&1|Out-Null;Write-Host '  OK: Hibernate re-enabled';$ok++}
}catch{Write-Host "  ERROR: Hibernate -- $($_.Exception.Message)";$err++}

# 10 -- C-State cap
Write-Host '-- [10/17] C-State cap at C1'
try {
    if(-not $hasPowercfg){Write-Host '  SKIP: powercfg missing';$warn++}
    elseif(-not $Undo){
        ...
        & powercfg -setacvalueindex SCHEME_CURRENT SUB_PROCESSOR IdleDemoteThreshold 40 2>&1|Out-Null
        & powercfg -setacvalueindex SCHEME_CURRENT SUB_PROCESSOR IdlePromoteThreshold 60 2>&1|Out-Null
        & powercfg -setdcvalueindex SCHEME_CURRENT SUB_PROCESSOR IdleDemoteThreshold 40 2>&1|Out-Null
        & powercfg -setdcvalueindex SCHEME_CURRENT SUB_PROCESSOR IdlePromoteThreshold 60 2>&1|Out-Null
        & powercfg -setactive SCHEME_CURRENT 2>&1|Out-Null;Write-Host '  OK: C-state restored';$ok++
    }
}catch{Write-Host "  ERROR: CState -- $($_.Exception.Message)";$err++}

# 11 -- CPU Tuning (AMD CPPC / Intel Hybrid)
Write-Host '-- [11/17] CPU tuning (AMD CPPC / Intel Hybrid)'
try {
    if($isAMD){
        ...
        if(-not $Undo){
            if(Test-Path $cp){Backup-RegValue -Path $cp -Name 'ACSettingIndex';Set-RegDWord -Path $cp -Name 'ACSettingIndex' -Value 2}
            if(Test-Path $cpp){Backup-RegValue -Path $cpp -Name 'ACSettingIndex';Set-RegDWord -Path $cpp -Name 'ACSettingIndex' -Value 1}
            if($hasPowercfg){& powercfg -setactive SCHEME_CURRENT 2>&1|Out-Null}
            Write-Host '  OK: AMD CPPC=2 CPPC-Preferred=1';$ok++
        }else{
            if(Test-Path $cp){Restore-RegValue -Path $cp -Name 'ACSettingIndex' -Type 'DWord'}
            if(Test-Path $cpp){Restore-RegValue -Path $cpp -Name 'ACSettingIndex' -Type 'DWord'}
            if($hasPowercfg){& powercfg -setactive SCHEME_CURRENT 2>&1|Out-Null}
            Write-Host '  OK: AMD CPPC restored';$ok++
        }
    }elseif($isIntel){
        ...
        if(Test-Path $hp){
            if(-not $Undo){Backup-RegValue -Path $hp -Name 'ACSettingIndex';Set-RegDWord -Path $hp -Name 'ACSettingIndex' -Value 0;if($hasPowercfg){& powercfg -setactive SCHEME_CURRENT 2>&1|Out-Null};Write-Host '  OK: Intel Heterogeneous=0 (P-cores first)';$ok++}
            else{Restore-RegValue -Path $hp -Name 'ACSettingIndex' -Type 'DWord';if($hasPowercfg){& powercfg -setactive SCHEME_CURRENT 2>&1|Out-Null};Write-Host '  OK: Restored';$ok++}
        }else{Write-Host '  SKIP: Non-hybrid Intel';$warn++}
    }else{Write-Host '  SKIP: Unknown CPU vendor';$warn++}
}catch{Write-Host "  ERROR: CPUTuning -- $($_.Exception.Message)";$err++}

# 12 -- HAGS
Write-Host '-- [12/17] HAGS (Hardware-Accelerated GPU Scheduling)'
try {
    $gp='HKLM:\SYSTEM\CurrentControlSet\Control\GraphicsDrivers'
    if(-not $Undo){Backup-RegValue -Path $gp -Name 'HwSchMode';Set-RegDWord -Path $gp -Name 'HwSchMode' -Value 2;$rebootNeeded=$true;Write-Host '  OK: HwSchMode=2 (reboot required)';$ok++}
    else{Restore-RegValue -Path $gp -Name 'HwSchMode' -Type 'DWord';Write-Host '  OK: HAGS restored';$ok++}
}catch{Write-Host "  ERROR: HAGS -- $($_.Exception.Message)";$err++}

# 13 -- RSS Core Steering
Write-Host '-- [13/17] RSS Core Steering (NIC interrupts off CPU 0)'
try {
    if($logCount -lt 4){Write-Host '  SKIP: <4 logical cores';$warn++}
    elseif($hasNetAdapterRss){
        $nics=Get-NetAdapter -Physical -EA SilentlyContinue|Where-Object{$_.Status -eq 'Up'}
        $nicCount=0
        foreach($nic in $nics){
            try{
                $rss=Get-NetAdapterRss -Name $nic.Name -EA SilentlyContinue;if($null -eq $rss){continue}
                if(-not $Undo){Ensure-BackupRoot;New-ItemProperty -Path $BackupRoot -Name "RSS_$($nic.Name)_BPN" -Value $rss.BaseProcessorNumber -Force -EA SilentlyContinue|Out-Null;Set-NetAdapterRss -Name $nic.Name -BaseProcessorNumber 2 -EA SilentlyContinue}
                else{$b=Get-ItemProperty -Path $BackupRoot -Name "RSS_$($nic.Name)_BPN" -EA SilentlyContinue;Set-NetAdapterRss -Name $nic.Name -BaseProcessorNumber (if($null -ne $b){$b."RSS_$($nic.Name)_BPN"}else{0}) -EA SilentlyContinue}
                $nicCount++
            }catch{}
        }
        Write-Host "  OK: RSS cmdlet -- $nicCount NIC(s)";$ok++
    }else{
        $ndisBase='HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e972-e325-11ce-bfc1-08002be10318}'
        $nicCount=0
        foreach($a in (Get-ChildItem -Path $ndisBase -EA SilentlyContinue|Where-Object{$_.PSChildName -match '^\d{4}$'})){
            ...
            if($desc -match 'WAN|Miniport|Loopback|Virtual|Hyper-V|TAP|Tunnel|Npcap|VMware'){continue}
        ...
        Write-Host "  OK: RSS registry fallback on $nicCount adapter(s)";$ok++
    }
}catch{Write-Host "  ERROR: RSS -- $($_.Exception.Message)";$err++}

# 14 -- BypassIO
Write-Host '-- [14/17] BypassIO (Win11 22H2+ NVMe volumes)'
try {
    if(-not $isWin11 -or $build -lt 22621){Write-Host '  SKIP: Win11 22H2+ required';$warn++}
    elseif(-not $hasFsutil){Write-Host '  SKIP: fsutil not found';$warn++}
    else{
        ...
        foreach($v in $vols){
            ...
            $bitlocker=$false
            try{$enc=Get-WmiObject -Namespace 'root\CIMV2\Security\MicrosoftVolumeEncryption' -Class Win32_EncryptableVolume -Filter "DriveLetter='$letter'" -EA SilentlyContinue;if($null -ne $enc -and $enc.ProtectionStatus -eq 1){$bitlocker=$true}}catch{$svc=Get-Service -Name 'fvevol' -EA SilentlyContinue;if($null -ne $svc -and $svc.Status -eq 'Running'){$bitlocker=$true}}
            if($bitlocker){Write-Host "  SKIP ${letter}: BitLocker active";continue}
            if(-not $Undo){& fsutil bypassio enable  $letter 2>&1|Out-Null}
            else          {& fsutil bypassio disable $letter 2>&1|Out-Null}
            $doneCount++
        }
        Write-Host "  OK: BypassIO $(if($Undo){'disabled'}else{'enabled'}) on $doneCount volume(s)";$ok++
    }
}catch{Write-Host "  ERROR: BypassIO -- $($_.Exception.Message)";$err++}

# 15 -- Large Pages (secedit primary, LSA P/Invoke fallback)
Write-Host '-- [15/17] Large Pages (SeLockMemoryPrivilege)'
try {
    $admSid='*S-1-5-32-544'
    if($hasSecedit){
        $exe="$env:SystemRoot\System32\secedit.exe"
        $inf="$env:SystemRoot\Temp\ppm_lp.inf";$db="$env:SystemRoot\Temp\ppm_lp.sdb";$log="$env:SystemRoot\Temp\ppm_lp.log"
        if(-not $Undo){
            & $exe /export /cfg $inf /quiet 2>&1|Out-Null
            if(Test-Path $inf){
                $c=[System.IO.File]::ReadAllText($inf)
                $line=[string]($c -split "`n"|Where-Object{$_ -match 'SeLockMemoryPrivilege'}|Select-Object -First 1)
                Ensure-BackupRoot;New-ItemProperty -Path $BackupRoot -Name 'LargePages_OrigLine' -Value $line -Force -EA SilentlyContinue|Out-Null
                if($line -match [regex]::Escape($admSid)){Write-Host '  SKIP: Already granted';$ok++}
                else{
                    $newLine=if([string]::IsNullOrWhiteSpace($line)){"SeLockMemoryPrivilege = $admSid"}else{$line.TrimEnd()+","+$admSid}
                    [System.IO.File]::WriteAllText($inf,($c -replace [regex]::Escape($line),$newLine))
                    & $exe /configure /db $db /cfg $inf /log $log /quiet 2>&1|Out-Null
                    Write-Host '  OK: SeLockMemoryPrivilege granted (secedit)';$ok++
                }
            }else{Write-Host '  WARN: secedit export failed';$warn++}
        }else{
            & $exe /export /cfg $inf /quiet 2>&1|Out-Null
            if(Test-Path $inf){
                $backed=Get-ItemProperty -Path $BackupRoot -Name 'LargePages_OrigLine' -EA SilentlyContinue
                if($null -ne $backed -and -not [string]::IsNullOrEmpty($backed.LargePages_OrigLine)){
                    $c=[System.IO.File]::ReadAllText($inf);$cur=[string]($c -split "`n"|Where-Object{$_ -match 'SeLockMemoryPrivilege'}|Select-Object -First 1)
                    [System.IO.File]::WriteAllText($inf,($c -replace [regex]::Escape($cur),$backed.LargePages_OrigLine))
                    & $exe /configure /db $db /cfg $inf /log $log /quiet 2>&1|Out-Null;Write-Host '  OK: Restored (secedit)';$ok++
                }else{Write-Host '  WARN: No backup';$warn++}
            }
        }
        Remove-Item $inf,$db,$log -EA SilentlyContinue
    }else{
        Write-Host '  INFO: secedit unavailable -- LSA P/Invoke fallback'
        if(-not $Undo){
            try{
                Add-Type -TypeDefinition $lsaSource -EA Stop
                $r=[PPMLsa]::Grant('S-1-5-32-544','SeLockMemoryPrivilege')
                if($r -eq 0){Write-Host '  OK: SeLockMemoryPrivilege granted (LSA API)';$ok++}else{Write-Host "  WARN: LSA API returned $r";$warn++}
            }catch{Write-Host "  WARN: LSA fallback failed: $($_.Exception.Message)";$warn++}
        }else{Write-Host '  INFO: secedit restore unavailable -- privilege persists until next secedit cycle';$warn++}
    }
}catch{Write-Host "  ERROR: LargePages -- $($_.Exception.Message)";$err++}

# 16 -- SSD TRIM
Write-Host '-- [16/17] SSD TRIM (retrim all fixed volumes)'
try {
    if(-not $Undo){
        $vols=Get-WmiObject Win32_Volume -EA SilentlyContinue|Where-Object{$_.DriveType -eq 3 -and $null -ne $_.DriveLetter}
        $trimCount=0
        foreach($v in $vols){
            ...
            if($hasOptimizeVol){try{Optimize-Volume -DriveLetter $drove -ReTrim -EA SilentlyContinue;$trimCount++}catch{}}
            else{$def=Get-Command defrag.exe -EA SilentlyContinue;if($null -ne $def){& defrag.exe "${drove}:" /X /U 2>&1|Out-Null;$trimCount++}}
        }
        Write-Host "  OK: TRIM on $trimCount volume(s) via $(if($hasOptimizeVol){'Optimize-Volume'}else{'defrag.exe /X'})";$ok++
    }else{Write-Host '  SKIP: TRIM is one-directional';$ok++}
}catch{Write-Host "  ERROR: SSDTrim -- $($_.Exception.Message)";$err++}

# =============================================================================
# 17 -- IFEO PRIORITY BOOST + JUNK CLEANUP
# =============================================================================
Write-Host '-- [17/17] IFEO Priority Boost (Above Normal -- browsers, Office, QB, Autodesk)'
try {
    ...
    foreach($exe in $targetExes){
        ...
        if(-not $Undo){
            # Backup (never fails the count)
            $eCpu=(Get-ItemProperty -Path $pk -Name 'CpuPriorityClass' -EA SilentlyContinue).CpuPriorityClass
            $eIo=(Get-ItemProperty -Path $pk -Name 'IoPriority' -EA SilentlyContinue).IoPriority
            $cpuBv=if($null -ne $eCpu){$eCpu}else{$SENTINEL}; $ioBv=if($null -ne $eIo){$eIo}else{$SENTINEL}
            New-ItemProperty -Path $ifeoBack -Name "${exe}_Cpu" -Value $cpuBv -Type DWord -Force -EA SilentlyContinue|Out-Null
            New-ItemProperty -Path $ifeoBack -Name "${exe}_Io"  -Value $ioBv  -Type DWord -Force -EA SilentlyContinue|Out-Null
            try{
                $regPath="HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\$exe\PerfOptions"
                & reg.exe add $regPath /v CpuPriorityClass /t REG_DWORD /d $CPU_ABOVE /f 2>&1|Out-Null
                & reg.exe add $regPath /v IoPriority /t REG_DWORD /d $IO_HIGH /f 2>&1|Out-Null
                $iok++
            }catch{$ierr++}
        }else{
            try{
                ...
                if(Test-Path $pk){
                    if($null -eq $bCpu -or $bCpu -eq $SENTINEL){
                        Remove-Item -Path $pk -Recurse -Force -EA SilentlyContinue
                        $parent=Split-Path $pk -Parent
                        $kids=Get-ChildItem -Path $parent -EA SilentlyContinue
                        $props=@(Get-ItemProperty -Path $parent -EA SilentlyContinue|Get-Member -MemberType NoteProperty -EA SilentlyContinue|Where-Object{$_.Name -notmatch '^PS'})
                        if((-not $kids)-and($props.Count -eq 0)){Remove-Item -Path $parent -Recurse -Force -EA SilentlyContinue}
                    }else{Set-ItemProperty -Path $pk -Name 'CpuPriorityClass' -Value $bCpu -Type DWord -EA SilentlyContinue;Set-ItemProperty -Path $pk -Name 'IoPriority' -Value $bIo -Type DWord -EA SilentlyContinue}
                }
                $iok++
            }catch{$ierr++}
        }
...
# Junk cleanup (same as Safe)
Write-Host '-- Junk file cleanup'
if(-not $Undo){
    try{
        ...
        foreach($prof in $profKeys){
            ...
            if(Test-Path $tp){Get-ChildItem -Path $tp -Filter 'thumbcache_*.db' -Force -EA SilentlyContinue|ForEach-Object{try{Remove-Item -LiteralPath $_.FullName -Force -EA SilentlyContinue}catch{}}}
            Remove-FolderContents -Path "$pp\AppData\Local\Microsoft\Windows\WER"
            # Teams cache (age-gated on Server)
            $teamsAge=if($isServer){120}else{0}
            foreach($tp2 in @("$pp\AppData\Roaming\Microsoft\Teams\Cache","$pp\AppData\Roaming\Microsoft\Teams\blob_storage","$pp\AppData\Roaming\Microsoft\Teams\GPUCache")){Remove-FolderContents -Path $tp2 -MinAgeMins $teamsAge}
        }
...
}else{Write-Host '  SKIP: Cleanup not reversed';$ok++}

# =============================================================================
# RESULT
# =============================================================================
Write-Host ''
Write-Host "=== PPM RMM Advanced $action complete: ok=$ok warn=$warn errors=$err ==="
if($rebootNeeded -and -not $Undo){Write-Host 'NOTE: Reboot recommended (HAGS, scheduler changes).'}
...
Set-PPMRMMUDF -Slot $UDFSlotN -Value "PPM-Adv $action $status"
...
Write-Host 'SUCCESS: PPM Advanced complete.'
exit 0
