...
<#
.SYNOPSIS
    Disk Space Recovery [WIN]
    Paladin Business Consulting | Datto RMM Component

.DESCRIPTION
    Recovers disk space by cleaning safe system and user temp locations.
    Designed to run as NT AUTHORITY\SYSTEM via Datto RMM.
    Server-aware: skips risky phases automatically on Windows Server OS.

    CLEANS (safe, system-managed locations only):
      - Windows\Temp                        (system temp -- all files)
      - Windows\SoftwareDistribution\Download  (cached Windows Update packages)
      - Windows\Prefetch                    (prefetch cache -- OS rebuilds automatically)
      - Windows\Logs                        (CBS, DISM, setup logs -- OS regenerates)
      - Windows\Minidump + MEMORY.DMP       (crash dumps)
      - Delivery Optimization cache         (Windows Update peer cache)
      - WinSxS backup components            (DISM /StartComponentCleanup -- OS safe)
      - All user AppData\Local\Temp         (per-user temp -- files >1hr old only)
      - All user Teams cache                (blob_storage, Cache, databases -- Teams rebuilds)
      - All user Recent shortcuts           (AppData\Roaming\Microsoft\Windows\Recent)
      - Recycle Bin                         (contents the user already deleted)
      - Windows Error Reporting dumps       (WER crash dumps -- no user data)
      - Thumbnail cache                     (per-user -- OS rebuilds automatically)
      - Windows.old / $WINDOWS.~BT / $WINDOWS.~WS  (upgrade folders -- flagged)
    ...
    NEVER TOUCHES:
      ...
      AppData\Roaming (except \Microsoft\Windows\Recent shortcuts),
      AppData\Local (except \Temp and Teams cache subfolders),
      Program Files, any user-created content.

    Paladin Business Consulting | Internal Use Only
    Version: 1.2.0 | Min OS: Windows 10 / Server 2016
#>
...
Write-Host '======================================================='
Write-Host ' Paladin Disk Space Recovery v1.2.0'
Write-Host ' Datto RMM | NT AUTHORITY\SYSTEM'
Write-Host '======================================================='
Write-Host "  OS      : $osCaption"
if ($script:IsServer) {
    Write-Host '  Mode    : SERVER -- MSI patch cache, font cache skipped'
} else {
...
# Font cache -- skipped on Server (causes RDS rendering glitch for active sessions)
if ($script:IsServer) {
...
# MSI Patch Cache -- skipped on Server (needed for role/feature repair)
if ($script:IsServer) {
...
if (!$profiles -or $profiles.Count -eq 0) {
    ...
    foreach ($u in $profiles) {
        ...
        # Teams cache -- on RDS/Server only clear if no active session (age-gated 2hr)
...
if ($script:ExitCode -eq 0) {
    Write-Host "SUCCESS: Disk cleanup complete. Recovered $(Format-GB $netGain) on $sysDrive."
} else {
